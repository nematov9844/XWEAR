...
export default function ProductDetails() {
  ...
  const handleLike = () => {
    // Agar mahsulot allaqachon yoqilgan bo'lsa, uni olib tashlash
    console.log(isLiked);
    
    if (isLiked) {
      dispatch(removeLike({ id: parseInt(id) })); // Agar yoqilgan bo‘lsa, olib tashlaydi
      console.log("Removed like:", likedItems); // Qo'shilgan console.log
    } else {
      dispatch(addLike(data)); // Agar yoqilmagan bo‘lsa, qo‘shadi
      console.log("Added like:", likedItems); // Qo'shilgan console.log
    }
  };
			...
			<div className='flex gap-8'>
				...
				<div className='w-2/3'>
					...
					<div className='flex gap-6 mt-6 items-center'>
						...
						<button
							onClick={handleLike}
							className={`px-6 py-3 rounded-lg border-2 ${isLiked ? "bg-red-600 text-white" : "bg-gray-500 text-gray-200"} border-gray-300 hover:bg-opacity-80 transition-all duration-300`}>
							{isLiked ? "Unlike" : "Like"}
						</button>
					</div>
				</div>
